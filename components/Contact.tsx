...
export default function Contact() {
  ...
  return (
    <div ref={sectionRef} className="w-full md:not-even:bg-[#151516] px-2 py-6 rounded-4xl" id="contact">
      <div className="w-full rounded-[28px] md:bg-[#1b1b1c] md:px-10 py-10 sm:py-22">
        <div className="flex items-start justify-between gap-6">
          ...
          <div className="contact-badge hidden sm:flex items-center gap-2 rounded-full border border-white/[0.08] bg-black/30 px-3 py-1.5 shrink-0 font-mono text-[11px] tracking-wider uppercase">
            <span className="relative flex h-1.5 w-1.5">
              {isOnline && <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[#5b4cf5]/60" />}
              <span
        ...
        <h3 className="contact-headline mt-10 text-white text-4xl sm:text-5xl md:text-[52px] font-semibold leading-[1.08] tracking-tight max-w-2xl">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-12 gap-10 border-t border-white/[0.06] pt-10">
                ...
                <div key={i} className="contact-channel group flex items-center justify-between py-4 border-b border-white/[0.06] hover:border-white/20 transition-colors">
                  ...
                  {c.label === "Email" ? (
                    <button type="button" onClick={copyEmail} className="flex items-center gap-1.5 text-white/30 hover:text-white text-xs font-mono uppercase tracking-wider transition-colors">
                      {copied ? (
                        <>
                          <svg className="h-3 w-3 text-[#5b4cf5]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
          ...
          <div className="md:col-span-7">
            {status === "success" ? (
              <div
                ref={successRef}
                className="h-full flex flex-col items-start justify-center rounded-2xl border border-[#5b4cf5]/30 bg-[#5b4cf5]/[0.06] p-8 min-h-[360px]"
              >
                ...
                <div className="success-text">
                  ...
                  <button type="button" onClick={() => setStatus("idle")} className="mt-6 text-[#5b4cf5] text-sm font-medium hover:underline underline-offset-4">
              ...
              <form onSubmit={handleSubmit} className="contact-form flex flex-col gap-7">
                ...
                {status === "error" && (
                  <p className="text-red-400 text-sm">
                    Something went wrong — try again, or reach out on one of the channels to the left.
                  </p>
                )}
                ...
                <button
                  ...
                  className="contact-field group mt-2 flex items-center justify-center gap-2 rounded-full bg-gradient-to-br from-[#5B4CF5] via-[#232322] [#5145EA] to-lime-400 bg[#5b4cf5] py-3.5 text-white text-sm font-medium hover:bg-[#4c3fe0] transition-colors disabled:opacity-60 disabled:cursor-not-allowed focus:outline-none focus-visible:ring-2 focus-visible:ring-[#5b4cf5] focus-visible:ring-offset-2 focus-visible:ring-offset-[#1b1b1c]"
                >
                  {status === "sending" ? (
                    <>
