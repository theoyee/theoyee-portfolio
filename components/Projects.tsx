...
export default function Projects() {
  ...
  return (
    <section className="w-full px-2 py-6" id="projects">
      <div className="w-full rounded-[28px] boder borderwhite/[0.06] bg[#1b1b1c] py-10 sm:py-12">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-white/[0.06] pt-10">
          {/* Zentra Degen Scanner */}
          <a
            href="https://zentra-degen.vercel.app/"
            target="_blank"

            className="group rounded-2xl border border-white/[0.06] bg-[#151516] overflow-hidden hover:border-white/20 transition-colors flex flex-col"
          >
            <div className="relative h-52 bg-[#0c0c0d] overflow-hidden">
              ...
              <svg
                viewBox="0 0 400 120"
                className="absolute bottom-0 left-0 w-full h-24 opacity-80"
              ...
              <div className="absolute bottom-4 right-4 font-mono text-[11px] text-[#f5a623]/70">
                $DEGEN +182.4%
              </div>
            </div>
            ...
            <div className="p-6 flex flex-col flex-1">
              ...
              <p className="mt-2 text-white/50 text-sm leading-relaxed flex-1">
          ...
          >
            <div className="relative h-52 bg-gradient-to-br from-[#2a2420] to-[#1c1815] overflow-hidden flex items-center justify-center">
              <span
                className="font-serif text-[120px] leading-none text-[#e8c07d]/10 select-none"
                aria-hidden
              >
                R
              </span>
        ...
        {/* Secondary directory list */}
        <div className="mt-6 border-t border-white/[0.06]">
          {secondary.map((p, i) => (
            <a
              key={i}
              href={p.href}
              className="group relative flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6 border-b border-white/[0.06] py-7 hover:bg-white/[0.02] transition-colors"
            >
              <span className="text-white/20 font-mono text-sm font-semibold w-8 shrink-0">
                {p.num}
              </span>
              ...
              <div className="flex-1">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {p.tags.map((tag, j) => (
                    <span
                      key={j}
                      className="text-[11px] font-mono uppercase tracking-wider text-white/35 border border-white/[0.08] rounded-full px-2.5 py-1"
                    >
                      {tag}
                    </span>
                  ))}
