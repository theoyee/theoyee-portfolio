...
export default function Services() {
  ...
  return (
    <section className=" w-full py-10 sm:p-12 px-2" id="services">
      ...
      <div className="mt-10 max-w-3xl">
        ...
        <p className="mt-6 text-white/50 text-base sm:text-lg leading-relaxed max-w-2xl">
      ...
      <div className="mt-12 border-t border-white/[0.06]">
        {offerings.map((service, i) => (
          <div
            key={i}
            className="group relative border-b border-white/[0.06] py-8 sm:py-10 transition-colors"
          >
            {/* left accent bar, grows in on hover */}
            <span className="absolute left-0 top-0 h-full w-px bg-lime-400 scale-y-0 group-hover:scale-y-100 origin-top transition-transform duration-300" />

            <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 sm:gap-6 sm:pl-6">
              {/* index + icon */}
              <div className="sm:col-span-2 flex items-center gap-3 sm:block">
                <span className="text-white/20 font-mono text-sm font-semibold block">
                  {service.num}
                </span>
                <div className="mt-0 sm:mt-3 h-10 w-10 rounded-xl bg-white/5 flex items-center justify-center border border-white/10 text-lime-500">
                  {service.icon}
                </div>
              </div>
              ...
              <div className="sm:col-span-8">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {service.tags.map((tag, j) => (
                    <span
                      key={j}
                      className="text-[11px] font-mono uppercase tracking-wider text-white/40 border border-white/[0.08] rounded-full px-2.5 py-1"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="sm:col-span-2 flex items-start sm:justify-end">
                <span className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-white/30 group-hover:text-white group-hover:border-lime-600 group-hover:bg-lime-400/10 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all">
      ...
      {/* Bottom CTA block */}
      <div className="mt-12 py-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <p className="text-white/40 text-sm max-sm:text-center max-sm:text-center">
          Have a project in mind that requires engineering focus?
        </p>
        <a
          href="#contact"
